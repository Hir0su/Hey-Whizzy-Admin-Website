...
def fetch_form():
    ...
    if keywords:
        ...
        for keyword in keywords:
            ...
            condition_without_special_chars = f"MATCH(e.question) AGAINST ('{keyword}' IN NATURAL LANGUAGE MODE)"
            condition_with_special_chars = f"MATCH(e.question) AGAINST ('{escaped_keyword}' IN NATURAL LANGUAGE MODE)"
        ...
        if category_id:
            ...
            cur.execute(f"SELECT e.file_path, e.file_name, e.question, e.answer, e.category_id "
                        f"FROM entries e "
                        f"WHERE ({conditions_str}) AND e.category_id = %s",
                        (category_id,))
        else:
            cur.execute(f"SELECT e.file_path, e.file_name, e.question, e.answer, e.category_id "
                        f"FROM entries e "
        ...
        cur.close()
        return render_template('index.html', entries=search_results, query=query)
    else:
        if category_id:
            # Fetch entries for the selected category if no search query is provided
